...
export default new Vuex.Store({
  ...
  actions: {
    ...
    fetchCinema({state, commit},cinemaId){
      return new Promise((resolve) => {
          ...
          if(cinema)
            ...
            fetch('http://localhost:8000/api/theatres/' + cinemaId)
            .then( obj => obj.json() )
              .then( res => {
                  commit('addMovie', res);
                  resolve(res);
    ...
    fetchShowtime({state, commit},showtimeId){
      return new Promise((resolve) => {
          ...
          if(showtime)
            ...
            fetch('http://localhost:8000/api/showtimes/' + showtimeId)
            .then( obj => obj.json() )
              .then( res => {
    ...
    fetchReservedSeats({commit},showtimeId){
          fetch('http://localhost:8000/api/seatshowtime/' + showtimeId)
            .then(res => res.json())
              .then( res =>{
    ...
    getUserData({state}){
      return new Promise((resolve) => {
        fetch('http://localhost:8000/api/users/' + state.user.userId, {
          method: 'GET',
          headers: { 'Content-Type': 'application/json', 'Authorization': `Bearer ${state.token}`}
        }).then( res => res.json() )
          .then( data => {
